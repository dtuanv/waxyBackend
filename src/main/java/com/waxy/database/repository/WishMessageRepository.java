...
public interface WishMessageRepository extends JpaRepository<WishMessage, Long> {
    ...
    @Query(value="SELECT * FROM wish_message WHERE today LIKE ?1 AND birth_user_id = ?2 AND message != ''",nativeQuery = true)
    Set<WishMessage> findWishMessageToMe(String today, long userId);
}
